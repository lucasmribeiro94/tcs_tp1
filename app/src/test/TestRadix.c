...
TEST(Radix, TestRadixSucesso1)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
  status = sort(arr, SMALL, (char *)"On", RADIX);
...
TEST(Radix, TestRadixSucesso2)
{
  len = sizeof(int) * MEDIUM;
  arr = (int *)malloc(len); 
  memcpy(arr, mUnsorted, len);
  status = sort(arr, MEDIUM, (char *)"On", RADIX);
...
TEST(Radix, TestRadixSucesso3)
{
  len = sizeof(int) * LARGE;
  arr = (int *)malloc(len); 
  memcpy(arr, lUnsorted, len);
  status = sort(arr, LARGE, (char *)"On", RADIX);
...
TEST(Radix, TestRadixTamanhoFalha1)
{
  len = sizeof(int) * TINY;
  arr = (int *)malloc(len); 
  memcpy(arr, tSorted, len);
  status = sort(arr, TINY, (char *)"On", RADIX);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Radix, TestRadixTamanhoFalha2)
{
  len = sizeof(int) * XLARGE;
  arr = (int *)malloc(len); 
  memcpy(arr, xlSorted, len);
  status = sort(arr, XLARGE, (char *)"On", RADIX);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Radix, TestRadixTamanhoFalha3)
{
  len = sizeof(int) * XXLARGE;
  arr = (int *)malloc(len); 
  memcpy(arr, xxlSorted, len);
  status = sort(arr, XXLARGE, (char *)"On", RADIX);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Radix, TestRadixTipoFalha1)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
...
TEST(Radix, TestRadixTipoFalha2)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
...
TEST(Radix, TestRadixTipoFalha3)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
...
TEST(Radix, TestRadixTipoFalha4)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
...
TEST(Radix, TestRadixTipoFalha5)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
  status = sort(arr, SMALL, (char *)"On2", RADIX);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Radix, TestRadixTipoFalha6)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
  status = sort(arr, SMALL, (char *)"Onlogn", RADIX);
