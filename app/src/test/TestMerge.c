...
TEST(Merge, TestMergeSucesso1)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
  status = sort(arr, SMALL, (char *)"Onlogn", MERGE);
...
TEST(Merge, TestMergeSucesso2)
{
  len = sizeof(int) * MEDIUM;
  arr = (int *)malloc(len); 
  memcpy(arr, mUnsorted, len);
  status = sort(arr, MEDIUM, (char *)"Onlogn", MERGE);
...
TEST(Merge, TestMergeSucesso3)
{
  len = sizeof(int) * LARGE;
  arr = (int *)malloc(len); 
  memcpy(arr, lUnsorted, len);
  status = sort(arr, LARGE, (char *)"Onlogn", MERGE);
...
TEST(Merge, TestMergeTamanhoFalha1)
{
  len = sizeof(int) * TINY;
  arr = (int *)malloc(len); 
  memcpy(arr, tSorted, len);
  status = sort(arr, TINY, (char *)"Onlogn", MERGE);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Merge, TestMergeTamanhoFalha2)
{
  len = sizeof(int) * XLARGE;
  arr = (int *)malloc(len); 
  memcpy(arr, xlSorted, len);
  status = sort(arr, XLARGE, (char *)"Onlogn", MERGE);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Merge, TestMergeTamanhoFalha3)
{
  len = sizeof(int) * XXLARGE;
  arr = (int *)malloc(len); 
  memcpy(arr, xxlSorted, len);
  status = sort(arr, XXLARGE, (char *)"Onlogn", MERGE);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Merge, TestMergeTipoFalha1)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
...
TEST(Merge, TestMergeTipoFalha2)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
...
TEST(Merge, TestMergeTipoFalha3)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
...
TEST(Merge, TestMergeTipoFalha5)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
  status = sort(arr, SMALL, (char *)"On", MERGE);
  
  TEST_ASSERT_EQUAL(1, status);
  free(arr);
}

TEST(Merge, TestMergeTipoFalha6)
{
  len = sizeof(int) * SMALL;
  arr = (int *)malloc(len); 
  memcpy(arr, sUnsorted, len);
  status = sort(arr, SMALL, (char *)"On2", MERGE);
